...
import { BaseResult, ErrorMessage, StyleAgeKeyProps, StyleAgeKeyTestProps } from './types';
import transations from '../translation.json';
import { ageKeyStyleContainer, ageKeyStyleContainerHover, ageKeyPlusIcon, ageKeyIcon, ageKeyNumber, ageKeyLoaderContainer, ageKeyLoader, agekeyStyleTestContainer, ageKeyTestStyleContainerHover } from './style';
...
const AgeKeyIconSVG: React.FunctionComponent<{ age?: number, fill?: string }> = ({ age, fill }) => {
  return (
...
export const AgeKeyTestStyleComponent: React.FC<StyleAgeKeyTestProps> = ({ language, isLoading }) => {
  const [text, setText] = useState("");
  const baseStyle = { ...ageKeyStyleContainer, ...agekeyStyleTestContainer }
...
type ErrorMessages = Record<number, ErrorMessage> & { default: 'Failed to complete ceremony' };
